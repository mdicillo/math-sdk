...
class GameExecutables(GameCalculations):
    ...
    def place_wild_frame(self, tuning: dict) -> None:
        """WILD STRIKE's frame, settled before the reels stop: on a landed WILD with the tuning's frame_on_wild chance,
        then every landed WILD beams; otherwise on a cell without a WILD (on a board of nothing but WILDs it can only
        settle on one)."""
        wilds, others = [], []
        for reel in range(self.config.num_reels):
            for row in range(self.config.num_rows[reel]):
                (wilds if self.board[reel][row].check_attribute("wild") else others).append({"reel": reel, "row": row})
        on_wild = random.random() < tuning["frame_on_wild"]
    ...
    def run_wild_strike(self, tuning: dict) -> None:
        """WILD STRIKE: only when WILDs beam, likelier the more beam, aimed to pay between the floor and the ceiling.
        The new wilds go onto the board before the lines evaluate."""
        self.strike_wilds = []
        if not self.beams:
            return
        chances = tuning["chance_by_beams"]
        if random.random() >= chances[min(len(self.beams), len(chances)) - 1]:
            return
